...
export default class SignaturePad extends LightningElement {
    ...
    @api existingInputText = "";
    @api existingSignature = null;

    async loadExistingData() {
        if (this.existingInputText) {
            this.inputText = this.existingInputText;
        }

        if (this.existingSignature) {
            const srcMatch = this.existingSignature.match(/src="([^"]*)"/);
            if (srcMatch && srcMatch[1]) {
                const decodedUrl = decodeURIComponent(srcMatch[1].replace(/&amp;/g, "&"));
                const img = new Image();
                ...
                img.onload = () => {
                    ...
                    const centerX = (this.canvas.width - imgWidth) / 2;
                    const centerY = (this.canvas.height - imgHeight) / 2;

                    this.context2d.drawImage(img, centerX, centerY, imgWidth, imgHeight);
                };
                img.onerror = (error) => {
                    console.log('Failed to load the image from the provided tag.', error, JSON.stringify(error));
                };
            } else {
                console.error('Invalid image tag: No src attribute found.');
            }
        }
    }
