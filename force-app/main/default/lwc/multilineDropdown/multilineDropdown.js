...
const delay = 350;
export default class MultilineDropdown extends LightningElement {

    //functional properties
    @api fieldLabel;
    @api disabled = false;
    @api searchAllowed = false;
    ...
    handleOutsideClick = (event) => {
        if (!this.template.contains(event.target)) {
            this.toggleOpenDropDown(false);  // Close the dropdown
        }
    }
    ...
    getLabel(value) {
        console.log('this.options', this.options, JSON.stringify(this.options));
    ...
    toggleOpenDropDown(toggleState) {
        ...
        if (toggleState) {
            if (!this.isListenerAdded) {  // Only add listener if not already added
                setTimeout(() => {
            ...
            if (this.isListenerAdded) {  // Remove listener if it was previously added
                setTimeout(() => {
